...
function resizeInteraction(thewidth,theheight) {
	...
	if (generalStyles.headerActive == 2){
		...
		$(".scroll-pane-content").css('height',(((revealHeight-headerActiveSize)-130)));
	}
	...
	$("#Search_Result_Header").css('left',(pyramidHolderW)+(55*scale));
	$(".scroll-pane-content").css('left',(pyramidHolderW)+(65*scale));
	...
	if (phrase.length < 1) { return; }
		...
		var count = 0;
		  $(".keyContext").each(function (i, v) {
				...
				var block = $(v);
				//if (i != 0) {
				block.html( block.text().replace( new RegExp(phrase, "gi"),	function (match) {
							count++;
							
							scrollContextTo($(v).attr('id'));
