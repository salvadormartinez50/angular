...
var app = window.angular.module('revalidaciones', ['ngResource']);

app.controller('revalidacionesController', ['$scope', '$http', '$window', '$rootScope',
    function ($scope, $http, $window, $rootScope) {
        ...
        $scope.getConsultaByFOLIO = function () {
            ...
            consulta = consulta.toUpperCase();
                ...
                $http({
                    method: 'GET',
                    url: global_uri+'/certificado/consultaRevalidaciones/?folio='+consulta,
                ...
                data.datos.forEach(function(fecha){

                $scope.fecEmision = data.datos[0].fecEmision;
                //console.log($scope.fecEmision);

            var fecha = new Date(fecha.fecEmision);
            ...
            //console.log(fecha)
            //fecha.fechaFin = [fecha.getDate(),(fecha.getMonth()+1), fecha.getFullYear()].join('/');
          // fecha.fechaFin = [dd.getDate(), '0'+(mm.getMonth()+1), fecha.getFullYear()].join('/');
           fecha.fechaFin = dd + '/' + mm + '/' + yyyy;
            //console.log(fecha.fechaFin)
            $scope.fechaEmision = fecha.fechaFin;
                          });


                $scope.activoEstatus;
                $scope.rechazObservaciones;
                data.datos.forEach(function(activo){
                              if(activo.estatus == 1){
                                $scope.activoEstatus = 'ACTIVO';   
                              }else{
                                  $scope.activoEstatus = 'RECHAZADO'; 
                                $scope.rechazObservaciones = activo.observaciones; 
                              }
                              

               // console.log($scope.activoEstatus);  
                          });
